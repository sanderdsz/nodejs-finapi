...
describe('GetStatementOperationController', () => {
  ...
  it('should be able to get user statement', async () => {
    ...
    expect(id).toBe(statement.body.id);
    expect(Number(amount)).toBe(Number(statement.body.amount));
    expect(description).toBe(statement.body.description);
  ...
  it('should not be able to get inexistent statement', async () => {
    ...
    const response = await request(app)
      .get('/api/v1/statements/f3f48ece-b225-441c-bc76-44fc9217cb88')
      .set({ Authorization: `Bearer ${token}` });

    expect(response.status).toBe(404);
  });

  it('should not be able to get statement of inexistent user', async () => {
    const responseToken = await request(app).post('/api/v1/sessions').send({
      email: 'user@finapi.com.br',
      password: 'user123',
    });
